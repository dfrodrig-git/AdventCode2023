...
listLabels ='A,K,Q,J,T,9,8,7,6,5,4,3,2,J'
dictLabels = {}
...
class Hand() :
    ...
    def _handType(self) :
        print(f"Computing hand type: {self.hand}")
        if self.hand=='JJJJJ' :
            return 7
        handCounter_=dict(self.handCounter)
        
        jokers = 0
        if 'J' in handCounter_.keys() :           
            jokers = handCounter_.pop('J')
        
        distribution=sorted(handCounter_.values())
        
        distribution[-1] += jokers
